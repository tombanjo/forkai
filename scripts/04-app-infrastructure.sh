...
# Default CORS origin is the Cloud Storage domain (not the bucket path)
# Requests from Cloud Storage come from https://storage.googleapis.com
if [ -z "$3" ]; then
  export TF_VAR_cors_origin="https://storage.googleapis.com"
else
  export TF_VAR_cors_origin="$3"
fi
export TF_VAR_service_name=${4:-"chat-service"}
export TF_VAR_model_name=${5:-"gemini-2.0-flash-lite"}

echo "Using Model: ${TF_VAR_model_name}"
echo "Using service account authentication (Application Default Credentials)"
